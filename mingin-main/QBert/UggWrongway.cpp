...
namespace dae
{
	...
	UggWrongWay::UggWrongWay(const std::shared_ptr<GameObject>& gameObject, int nrRows, float cubesWidth, float cubesHeight,
	...
	bool UggWrongWay::MoveLeftUp()
	{
		if (m_IsFrozen == false)
		{
			if (m_CurrentCubeIdx != m_CurrentRow * (m_CurrentRow + 1) / 2 - m_CurrentRow + 1 && m_CurrentCubeIdx != 1)
			{
				...
				m_Subject->Notify(Event::UggWrongwayMoved);
				return true;
			}
			else
			{
				m_Alive = false;
				m_Subject->Notify(Event::UggWrongwayFell);
				return false;
			}
		}
		return false;
